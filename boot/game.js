...
function createCamera(program, operator) {
  ...
  const cameraPos   = matrix.vec3.fromValues(0.0, 0.0,  3.0);
...
function readSkeleton(filename, vertexCount) {
  ...
  var buf = new Int32Array(vertexCount * BIND_LEN);
  var ske = new Float32Array(SKE_SIZE*16);

  buf.fill(-1);
  var vbind = config.vbind;

  for (var t in vbind) {
    var data = config.bindex[t];
    var varr = vbind[t];
  ...
  for (var i=0; i<config.bone.length; ++i) {
    var j = i*16;
    matrix.mat4.copy(ske.subarray(j, j+16), config.bone[i]);
  }
...
function createAnimation(drawObj, skeObj) {
  ...
  gl.glBufferData(gl.GL_UNIFORM_BUFFER, skeObj.bind, gl.GL_STATIC_DRAW);
  gl.glVertexAttribPointer(2, 2, gl.GL_SHORT, gl.GL_FALSE, 2 * gl.sizeof$int, 0);
  gl.glEnableVertexAttribArray(2);
  
  var skeleton = drawObj.program.getUniform('skeleton');
  skeleton.setMatrix4fv(10, gl.GL_FALSE, skeObj.ske);

  var rightFoot = skeObj.ske.subarray(0, 16);
  ...
  //
  function draw(used) {
    // matrix.mat4.rotateX(rightFoot, rightFoot, used);
    // skeleton.setMatrix4fv(10, gl.GL_FALSE, skeObj.ske);
  }
}